...
def evaluate_cambridge_dataset(dataset_dir, scene_name, pose_file, output_dir, dino_config, dino_weights, sam_weights, text_prompts=None, device=None, frame_interval=1, max_pairs=0):
    ...
    for seq in sequences:
        ...
        for i in range(0, len(seq_images) - 1, frame_interval):
            if i + frame_interval < len(seq_images):
                ...
                # 检查是否达到最大处理对数限制
                if max_pairs > 0 and pairs_processed >= max_pairs:
                    break
                ...
                try:
                    ...
                    if T_estimated is not None:
                        ...
                        if method == 'standard':
                            ...
                            # 这是当前唯一的结果，也是最佳结果
                            results["best"]["rotation_errors"].append(rot_err)
                            results["best"]["translation_errors"].append(trans_err)
    ...
    for method in ["standard", "hybrid", "best"]:
        if results[method]["rotation_errors"]:
    ...
    for method in ["standard", "hybrid", "best"]:
        if method in summary:
            ...
            print(f"图像对数量: {summary[method]['pairs_count']}")
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="语义引导SIFT位姿估计 - Cambridge Landmarks 评估")
    ...
    parser.add_argument("--dataset-dir", type=str, default="",
                        help="Cambridge Landmarks数据集根目录")
    parser.add_argument("--scene", type=str, default="ShopFacade",
                        help="场景名称，例如：ShopFacade, KingsCollege等")
    ...
    if args.dataset_mode:
        ...
        if not args.pose_file:
            print("错误：请提供位姿文件路径 --pose-file")
            return
            
        evaluate_cambridge_dataset(
            args.dataset_dir, args.scene, args.pose_file, args.output_dir,
            dino_config, dino_weights, sam_weights,
            args.text_prompts, args.device, args.frame_interval, args.max_pairs
        )
    else:
        # 单对图像处理模式
