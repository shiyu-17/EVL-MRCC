# ​核心功能
# ​3D 投影：利用深度图和相机内参生成 3D 点云，投影到 2D 图像平面。
# ​边界框转换：将 3D 边界框角点投影到 2D，通过后处理确保其在图像范围内。
# ​多进程加速：使用 multiprocessing.Pool 并行处理多个视频，提升效率。
# ​关键函数
# generate_point()：将深度图转换为点云，支持世界坐标系或相机坐标系。
# get_2d_box()：通过 3D 角点投影生成 2D 边界框，过滤不可见区域。
# convert_annotation_2d_points()：基于点云的密度过滤生成物体 2D 注释（需至少 20 个点）。

# ​输出格式：保存为 2d_annotations_<video_id>.json，包含每帧的物体标签、UID 和边界框。
import sys
